import tkinter as tk
from tkinter import ttk, scrolledtext
from matplotlib.figure import Figure
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
from sympy import lambdify

from calculos import FunctionAnalyzer
from resolucion import build_explanation

class AnalyzerGUI:
    def __init__(self, root):
        self.root = root
        root.title("Analizador de Funciones (versión humana)")
        self._build_ui()
        self.current_analyzer = None
        self.current_eval = None

    def _build_ui(self):
        frm = ttk.Frame(self.root, padding=10)
        frm.grid(column=0, row=0, sticky="nsew")

        ttk.Label(frm, text="Función f(x):").grid(column=0, row=0, sticky="w")
        self.fn_entry = ttk.Entry(frm, width=40)
        self.fn_entry.grid(column=1, row=0)
        self.fn_entry.insert(0, "x^2")

        ttk.Label(frm, text="Evaluar en x:").grid(column=0, row=1, sticky="w")
        self.x_entry = ttk.Entry(frm, width=20)
        self.x_entry.grid(column=1, row=1)

        ttk.Button(frm, text="Analizar", command=self.on_analyze).grid(column=2, row=0)
        ttk.Button(frm, text="Evaluar", command=self.on_evaluate).grid(column=2, row=1)
        ttk.Button(frm, text="Graficar", command=self.on_plot).grid(column=2, row=2)
        ttk.Button(frm, text="Resolver", command=self.on_resolve).grid(column=2, row=3)
        ttk.Button(frm, text="Limpiar", command=self.on_clear).grid(column=2, row=4)

        self.output = scrolledtext.ScrolledText(frm, width=70, height=10)
        self.output.grid(column=0, row=5, columnspan=3)

        self.expl_output = scrolledtext.ScrolledText(frm, width=70, height=10)
        self.expl_output.grid(column=0, row=6, columnspan=3)

        self.fig = Figure(figsize=(5,4))
        self.ax = self.fig.add_subplot(111)
        self.canvas = FigureCanvasTkAgg(self.fig, master=frm)
        self.canvas.get_tk_widget().grid(column=3, row=0, rowspan=7)

    def on_analyze(self):
        fn_text = self.fn_entry.get().strip()
        self.current_analyzer = FunctionAnalyzer(fn_text)
        res = self.current_analyzer.analyze()
        self.output.insert("end", res['steps']+"\n")

    def on_evaluate(self):
        if not self.current_analyzer: return
        x_text = self.x_entry.get().strip()
        res = self.current_analyzer.evaluate_at(x_text)
        self.current_eval = res
        self.output.insert("end", res['steps']+"\n")

    def on_resolve(self):
        if not self.current_analyzer: return
        analysis = self.current_analyzer.analyze()
        text = build_explanation(self.current_analyzer, analysis, self.current_eval)
        self.expl_output.delete("1.0", "end")
        self.expl_output.insert("end", text)

    def on_plot(self):
        if not self.current_analyzer: return
        f_num = lambdify(self.current_analyzer.var, self.current_analyzer.expr, modules=["math"])
        xs, ys = [], []
        for i in range(-20, 21):
            xval = i/2
            xs.append(xval)
            try:
                yval = f_num(xval)
            except Exception:
                yval = float('nan')
            ys.append(yval)
        self.ax.clear()
        self.ax.plot(xs, ys, label=str(self.current_analyzer.expr))
        self.ax.axhline(0, color="black")
        self.ax.axvline(0, color="black")
        self.ax.legend()
        self.canvas.draw()

    def on_clear(self):
        self.output.delete("1.0", "end")
        self.expl_output.delete("1.0", "end")
        self.ax.clear()
        self.canvas.draw()
